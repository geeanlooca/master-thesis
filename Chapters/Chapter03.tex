...
% The emitted light is collected by connecting a fiber optic patch cord to the FC/APC connector on the front panel, which also includes two SMA connectors for the sweep-start trigger and the $k$-clock signal.
...
Using a photodiode it's possible to measure the istantaneous power profile of the laser emission. The photodiode generates an electrical current which is proportional to the power of the detected light: 
\begin{equation}
	I_{ph} = \mathcal{R} \cdot P 
\end{equation}
The constant $\mathcal{R}$ is called \emph{responsivity} [A/W] and is specific to the detector used. The current can then be measured with an oscilloscope. In order to avoid the saturation of the receiver, a 3 dB coupler was inserted between the source and the photodiode. The obtained power profile is depicted in \autoref{fig:axsun-power} along with the sweep trigger. We can observe that outside of the first $\sim 5$ $\mu$s after the positive edge of the trigger, the istantaneous power assumes an irregular profile. 
...
\begin{figure}[hbt]
	...
	\caption{Experimental estimation of the normalized coherence function \cite{Calabrese2017}.}\label{fig:axsun-coherence-length}
\end{figure}
...
\begin{equation}\label{eq:galvo-split-signals}
	v_i^+(t) = \begin{cases}
		v_i(t) & v_i(t) \geq 0\\
		0 & v_i(t) < 0
 	\end{cases},
...
Galvo system, collimator and scanning lens are housed in a cage system mount\footnote{\url{https://www.thorlabs.com/thorproduct.cfm?partnumber=GCM102/M}} and mounted on a movable vertical support. A photo of the final system is available in \autoref{fig:testa}.
...
\label{sec:daq}	
	...
	The board also comes with the proprietary AlazarDSO software to configure the device, acquire and display signals. For custom applications, the board can be integrated using a supplied \ac{SDK} which includes libraries and headers for a variety of different programming languages, including C/C++, Python and MATLAB. The device was installed in a Dell Precision T5810 workstation\footnote{\url{https://www.dell.com/us/business/p/precision-t5810-workstation/fs}} with the following specifications:
	\begin{itemize}
...
\begin{itemize}
	\item 2 Thorlabs TW1300R5A2 50:50 fiber couplers\footnote{\url{https://www.thorlabs.com/thorproduct.cfm?partnumber=TW1300R5A2}}
	\item 1 Thorlabs TW1300R2A2 90:10 fiber coupler\footnote{\url{https://www.thorlabs.com/thorproduct.cfm?partnumber=TW1300R2A2}}	
\end{itemize} 
...
For the design of the interferometer it is useful to determine the length of the coupler's arms with high precision. This was accomplished using an \acf{OFDR} and the setup in \autoref{fig:setup-couplers-length}. In particular, the OFDR used in this thesis is a OBR 4600 by Luna Innovations\footnote{\url{http://lunainc.com/product/sensing-solutions/obr-4600/}}.
